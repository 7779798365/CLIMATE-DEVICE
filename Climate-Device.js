...
let isAnimating = false; //Add Flag To Indicate iIf Animation Is Active
...
function changeTemp(element, newTemp) {
  ...
  let currentTemp =
    unit === "°F" ? currentTempF : Math.round(((currentTempF - 32) * 5) / 9);
  let finalTemp =
    unit === "°F" ? newTemp : Math.round(((newTemp - 32) * 5) / 9);

  let duration = 2000; //Duration Of The Animation In Milli Seconds
  let startTime = null;
  function animate(currentTime) {
    ...
    let tempNow = Math.round(
      currentTemp + progress * (finalTemp - currentTemp)
    );
